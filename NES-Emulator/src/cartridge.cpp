...
bool CARTRIDGE::load(std::string link){
    ...
    char buffer = 0x00;
    for(int i = 0; i < 8; i++) ROMfile.read(&buffer,1); //skip ines file header
    
    //number of 16kB rom banks (= 1 or 2)
    bool mirror_prgrom = false;
    ROMfile.read(&buffer,2);
    if(buffer == 0x01) mirror_prgrom = true;
    
#warning TODO handle vrom
    //number of 8kB vrom banks (= 1 or 2)
//    bool mirror_prgrom = false;
    ROMfile.read(&buffer,2);
//    if(buffer == 0x01) mirror_prgrom = true;
    
    //skip rest of header
    for(int i = 0; i < 4; i++) ROMfile.read(&buffer,1);
    
    if(prgrom){
        for(Address pos = 0x8000; ((int) pos) < 0xC000; pos++){ //load prgrom
